...
export class ProfileComponent implements OnInit {
  ...
  updateProfile(): void {
    ...
    if (this.username.trim().length < 3) {
      this.showProfileMsg('Username must be at least 3 characters.', 'error');
      return;
    }
    if (!this.isValidEmail(this.email.trim())) {
      this.showProfileMsg('Please enter a valid email address.', 'error');
      return;
    }
  ...
  changePassword(): void {
    ...
    if (this.newPassword === this.currentPassword) {
      this.showPasswordMsg('New password must be different from your current password.', 'error');
      return;
    }
    if (this.newPassword !== this.confirmPassword) {
      this.showPasswordMsg('New passwords do not match.', 'error');
